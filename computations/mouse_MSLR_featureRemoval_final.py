...
dat_train_final = dat_train_final[:,np.where(varNames_mouse!='EyeMov')[0]]
# # No pupil:
# dat_train_final = dat_train_final[:,1:]
# # Only pupil:
# dat_train_final = dat_train_final[:,0].reshape(-1,1)
## No eye quantities:
# dat_train_final = dat_train_final[:,[1,4,5,6,7]]
# No nose quantities:
# dat_train_final = dat_train_final[:,np.setdiff1d(np.arange(len(varNames_mouse)-1), [1,4,5])]
# # No whiskers quantities:
dat_train_final = dat_train_final[:,np.setdiff1d(np.arange(len(varNames_mouse)-1), [6,7])]
...
if args.doCV:
    ...
    for num_state in internal_states:
        ...
        if __name__ == '__main__':
            ...
            np.savez("Results/Mouse/Results_CV_sLDS_Optuna_mouse_"+str(numTrials)+"trials_"+str(num_state)+"_states_"+args.date+"_RT_noWhiskersFeaturesModel_AllSubjects_R2score_CVOnly.npz", 
            ...
            np.savez("Results/Mouse/Results_ParamImportance_sLDS_Optuna_mouse_"+str(numTrials)+"trials_"+str(num_state)+"_states_"+args.date+"_RT_noWhiskersFeaturesModel_AllSubjects_R2score_CVOnly.npz",    dict(most_important_parameters.items()))
    ...
    for rr in range(len(files_logs)):
        start2 = time.time()
        filename = f'RawData/Mouse/Predictors_emissions_mouse_test_session{rr}.npz'
        ...
        ## No eye quantities:
        # dat_test = dat_test[:,[1,4,5,6,7]]
        # No nose quantities:
        # dat_test = dat_test[:,np.setdiff1d(np.arange(len(varNames_mouse)-1), [1,4,5])]
        # # No whiskers quantities:
        dat_test = dat_test[:,np.setdiff1d(np.arange(len(varNames_mouse)-1), [6,7])]
        ...
        np.savez("Results/Mouse/Results_Test_sLDS_Optuna_mouse_"+str(numTrials)+"trials_"+args.date+"_RT_noWhiskersFeaturesModel_AllSubjects_session"+str(rr), results)
