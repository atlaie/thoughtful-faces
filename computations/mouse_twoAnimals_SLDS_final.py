...
varNames_mouse = np.array(['PupilSize', 'EyeMov', 'NoseMov', 'Eye [x]', 'Eye [y]', 'Nose [x]', 'Nose [y]', 'Whiskers [x]', 'Whiskers [y]'])

#dats_train = np.load('Predictors_emissions_mouse_concat_train_20pad.npz', allow_pickle = True)
dats_train = np.load('RawData/Mouse/Predictors_emissions_mouse_TwoAnimals_concat_train_20pad.npz', allow_pickle = True)

dat_train = dats_train['predictors']
dat_train = dat_train[:,np.where(varNames_mouse!='EyeMov')[0]]
...
dat_train_final = scaler.fit_transform(dat_train[:siz,:])
dat_val_final = scaler.transform(dat_train[siz:,:])

sessions_twoAnimals = np.load('RawData/Mouse/SessionNumbers_TwoAnimals.npy')
    
if args.doCV:
    acme.cluster_cleanup()
    client = acme.esi_cluster_setup(partition="16GBXS", n_jobs=int(nSplits),
    ...
    for num_state in internal_states:
        ...
        def objective(trial):
            ...
            macaque_mslr_instance = pipln.model_MSLR_concat(y_train_final[:siz,:],dat_train_final,y_train_final[siz:,:],dat_val_final,
        ...
        if __name__ == '__main__':
            ...
            np.savez("Results/Mouse/Results_CV_MSLR_Optuna_mouse_"+str(numTrials)+"trials_"+str(num_state)+"_states_"+args.date+"_RT_TwoAnimals_R2score_CVOnly.npz", 
            ...
            np.savez("Results/Mouse/Results_ParamImportance_MSLR_Optuna_mouse_"+str(numTrials)+"trials_"+str(num_state)+"_states_"+args.date+"_RT_TwoAnimals_R2score_CVOnly.npz",    dict(most_important_parameters.items()))
    ...
    scaler = StandardScaler()
    dat_train_final = scaler.fit_transform(dat_train)
    ...
    for rr in range(len(sessions_twoAnimals)):
        start2 = time.time()
        filename = f'RawData/Mouse/Predictors_emissions_mouse_test_session{sessions_twoAnimals[rr]}.npz'
        dats = np.load(filename, allow_pickle = True)
        dat_test = dats['predictors']
        dat_test = dat_test[:,np.where(varNames_mouse!='EyeMov')[0]]
        ...
        np.savez("Results/Mouse/Results_Test_MSLR_Optuna_mouse_"+str(numTrials)+"trials_"+args.date+"_RT_TwoAnimals_session"+str(sessions_twoAnimals[rr]), results)
